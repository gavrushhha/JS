...
app.get('/trains/all', async (_, res) =>{
  ...
  const queryRes = await pool.query<User>(query)
...
io.on('connection', (socket) => {
  ...
  socket.on('user_update', async (user: User) => {
    ...
    const queryRes = await pool.query<{id: number}>(query, [user.full_name, user.docs, user.id])
